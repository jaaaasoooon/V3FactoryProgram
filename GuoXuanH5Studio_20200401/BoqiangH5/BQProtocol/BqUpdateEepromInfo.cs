...
namespace BoqiangH5
{
    public partial class UserCtrlEEPROM : UserControl
    {
        ...
        public event EventHandler<EventArgs<bool>> CheckEepromParamOKEvent;
        public void BqUpdateRomInfo(List<byte> listRecv)
        {
            if (listRecv.Count < (listRecv[2] << 8 | listRecv[3]) || listRecv[0] != 0xCC || listRecv[1] != 0xB1)
            {
                return;
            }
            ...
            if (isCheckEeprom)
            {
                byte[] EepromData = new byte[26];
                Buffer.BlockCopy(listRecv.ToArray(), 1, EepromData, 0, EepromData.Length);

                Thread.Sleep(100);
                LoadPara(FilePath);
                byte[] romData = new byte[26];
                if(GetEEPROMDataBuf(romData))
                {
                    if(Enumerable.SequenceEqual(EepromData,romData))
                    {
                        CheckEepromParamOKEvent?.Invoke(this, new EventArgs<bool>(true));
                    }
                    else
                    {
                        CheckEepromParamOKEvent?.Invoke(this, new EventArgs<bool>(false));
                    }
                }
                else
                {
                    MessageBox.Show("获取Eeprom参数失败，请检查！", "获取Eeprom参数提示", MessageBoxButton.OK, MessageBoxImage.Information);
                }
                isCheckEeprom = false;
                return;
            }
            ...
            if (isShowMsg)
                AutoClosedMsgBox.Show("读取 EEPROM 参数成功！", "读取EEPROM提示", 1000,64);
        ...
        private void SetTxtBox_VoltageAndTemp(List<byte> listRecv)
        {
            ...
            double nRref = GetRrefVal(listReadRom[29]);

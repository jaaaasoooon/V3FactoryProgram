    ...
    public partial class UserCtrlEEPROM : UserControl
    {
        ...
        public void UpdateEepromWnd()
        {
            if (MainWindow.m_statusBarInfo.IsOnline)
            {
                ...
                //        btnWritePara_Click(null, null);//加载完直接下发参数
                //    }
                //}
            }
            else
        ...
        public void RequireWriteEeprom(string filePath,bool _isRequireWrite)
        {
            isShowMsg = false;
            FilePath = filePath;
            isRequireWriteEeprom = _isRequireWrite;
            ReadPara();
        }
        bool isCheckEeprom = false;
        public void RequireCheckEeprom(string filePath)
        {
            //LoadPara(filePath);
            FilePath = filePath;
            isShowMsg = false;
        ...
        private void LoadPara(string filePath)
        {
            ...
            {
                MessageBox.Show("参数加载失败！", "加载参数提示", MessageBoxButton.OK, MessageBoxImage.Information);
            }
        }
        private void btnLoadPara_Click(object sender, RoutedEventArgs e)
        {
            ...
            //string filename = string.Format("H5_EEPROM_{0}_config.dat",DateTime.Now.ToString("yyyyMMddHHmmss"));
        ...
        private void SavePara(string fileName)
        {
            try
            {
            ...
            {
                MessageBox.Show("EEPROM 参数保存失败! ", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
            }
        }
        ...
        {;
            U_ID = "默认值";
            //RequireReadUID();
            RequireReadUIDEvent?.Invoke(this, new EventArgs<string>("Eeprom"));
        }
        ...
        public void SetUID(string uid)
        {
            try
            {
                ...
                {
                    if (isRequireWriteEeprom)
                    {
                        WriteEepromDataOverEvent?.Invoke(this, new EventArgs<bool>(true));
                        isRequireWriteEeprom = false;
                    }
                    else
                    {
                        SaveOperationRecord(BoqiangH5Repository.CSVFileHelper.ToHexStrFromByte(BqProtocol.BqInstance.IssuedBytesList.ToArray()), OperationTypeEnum.Eeprom写入, OperationResultEnum.失败, "写入 EEPROM 参数失败！");
                        MessageBox.Show("写入 EEPROM 参数失败，请检查输入数据！", "写入 EEPROM 提示", MessageBoxButton.OK, MessageBoxImage.Error);
                    }
                    return;
                }
                BqProtocol.BqInstance.m_bIsStopCommunication = true;
                BqProtocol.BqInstance.SendMultiFrame(romData, 26, 0xA5);
                isWriteEeprom = true;
            }
            catch (Exception ex)
            {
        ...
        public event EventHandler<EventArgs<bool>> WriteEepromDataOverEvent;
        bool isRequireWriteEeprom = false;
            ...
            if(isWriteEeprom)
            {
                ...
                if (e.RecvMsg[0] == 0xDD || e.RecvMsg[1] == 0xA6 || e.RecvMsg.Count == (e.RecvMsg[2] << 8 | e.RecvMsg[3]))
                {
                    ...
                    if (res == 0)
                    {
                        ...
                        {
                            SaveOperationRecord(BoqiangH5Repository.CSVFileHelper.ToHexStrFromByte(BqProtocol.BqInstance.IssuedBytesList.ToArray()), OperationTypeEnum.Eeprom写入, OperationResultEnum.成功, string.Empty);
                            AutoClosedMsgBox.Show("写入 EEPROM 参数成功！", "写入 EEPROM 提示", 1000, 64);
                        }
                    }
                    else
                    {
                        if (isRequireWriteEeprom)
                        {
                            WriteEepromDataOverEvent?.Invoke(this, new EventArgs<bool>(false));
                            isRequireWriteEeprom = false;
                        }
                        else
                        {
        ...
        //    try
        //    {
        //        using (Eb12Entities eb12 = new Eb12Entities())
        //        {
